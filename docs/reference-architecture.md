...
The credentials file is re-read for each request, and a 401 response triggers one fresh read and retry (with in-process OAuth refresh by default; disable via `--no-oauth-refresh` or `OMNI_OAUTH_REFRESH=0`). Login still depends on Claude Code initially; the CLI is not in the request path.
