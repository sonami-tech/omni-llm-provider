...
mod tests {
    ...
    fn canonical_tool_choice_all_variants_serde() {
        for tc in [
            ...
            match (&tc, &back) {
                ...
                (
                    CanonicalToolChoice::Specific { name: n1 },
                    CanonicalToolChoice::Specific { name: n2 },
                ) => assert_eq!(n1, n2),
    ...
    impl LlmProvider for ClaudeMock {
        ...
        async fn send(&self, req: CanonicalRequest) -> Result<CanonicalResponse, ProviderError> {
            ...
            Ok(CanonicalResponse {
                ...
                tool_calls: if req.tools.is_some() {
    ...
    async fn llm_provider_send_roundtrips() {
        ...
        let base = CanonicalRequest {
            ...
            messages: vec![CanonicalMessage {
                role: "user".into(),
                content: CanonicalContent::Text("hi secret".into()),
            }],
            tools: None,
            tool_choice: None,
            max_tokens: None,
    ...
    async fn parity_same_canonical_request_for_grok_and_claude() {
        ...
        let req = CanonicalRequest {
            ...
            messages: vec![CanonicalMessage {
                role: "user".into(),
                content: CanonicalContent::Text("parity check".into()),
            }],
            tools: Some(vec![CanonicalTool {
                name: "t".into(),
                description: None,
    ...
    fn tools_and_specific_tool_choice_serde() {
        let tools = Some(vec![CanonicalTool {
            name: "search".into(),
    ...
    async fn parity_grok_claude_same_req_equiv_resp() {
        ...
        assert_eq!(rg.model, rc.model);
        assert!(
            rg.usage.input_tokens > 0 || !rg.tool_calls.is_empty() || !rc.tool_calls.is_empty()
        );
    }
    ...
    fn roundtrips_with_sim_replacements() {
        ...
        let req = CanonicalRequest {
            ...
            messages: vec![CanonicalMessage {
                role: "user".into(),
                content: CanonicalContent::Text(p),
            }],
            tools: Some(vec![CanonicalTool {
                name: "bar-tool".into(),
                description: None,
    ...
    fn more_content_and_message_serde() {
        let m = CanonicalMessage {
            role: "assistant".into(),
            content: CanonicalContent::Text("resp".into()),
        };
    ...
    async fn parity_includes_finish_and_usage() {
        let req = CanonicalRequest {
            ...
            messages: vec![CanonicalMessage {
                role: "user".into(),
                content: CanonicalContent::Text("q".into()),
            }],
            ..Default::default()
        };
        let rg = GrokMock.send(req.clone()).await.unwrap();
        ...
        assert_eq!(
            rg.usage.output_tokens + rc.usage.output_tokens,
            rg.usage.output_tokens + rc.usage.output_tokens
        );
    }
}
