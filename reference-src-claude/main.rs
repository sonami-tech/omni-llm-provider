...
use upstream::fingerprint::{FingerprintProfile, resolve_profile, valid_profile_selectors};
...
pub struct AppState {
    ...
    pub fingerprint_profile: &'static FingerprintProfile,
...
async fn main() {
    ...
    let fingerprint_profile = resolve_profile(&config.fingerprint_profile).unwrap_or_else(|| {
        error!(
            "Unknown Claude Code fingerprint profile {:?}. Valid profiles: {}",
    ...
    info!(
        profile = fingerprint_profile.name,
        claude_cli_version = fingerprint_profile.claude_cli_version,
        stainless_package_version = fingerprint_profile.stainless_package_version,
        stainless_runtime_version = fingerprint_profile.stainless_runtime_version,
        entrypoint = fingerprint_profile.entrypoint,
        "Using pinned Claude Code fingerprint"
    );
    ...
    let upstream_client = match upstream::UpstreamClient::new_with_profile(fingerprint_profile) {
        Ok(c) => c,
        Err(e) => {
    ...
    let api_routes = Router::new()
        // /v1/models serves BOTH SDKs: the handler returns the Anthropic-shaped
        // catalog when the request carries an `anthropic-version` header (the
        // Anthropic SDK always sends it), else the OpenAI shape. This keeps both
        // `openai` and `anthropic` `models.list()` working on the same path.
        ...
        // Native Anthropic Messages surface (stream vs non-stream via body field).
        .route("/v1/messages", post(routes::messages::messages_handler))
        .route(
            "/v1/messages/count_tokens",
            post(routes::messages::count_tokens_handler),
        )
