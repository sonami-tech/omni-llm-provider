...
Canonical target: Claude Code `2.1.142`, profile `cc-2.1.142-sdk-cli`.
...
Algorithm recovered for `2.1.142`:
...
CCP computes the checksum over the exact JSON bytes CCP sends. Those bytes do
...
- The active profile `cc-2.1.142-sdk-cli` owns the checksum behavior.
...
Rust unit tests in `src/upstream/fingerprint.rs` lock these fixtures in.
...
```sh
tools/fingerprint/check_claude_code_drift.py
```
