...
fixture for CCP's `x-anthropic-billing-header` cch checksum.

The Rust test `cch_matches_real_2_1_161_clean_room_capture_vectors`
(`src/upstream/fingerprint.rs`) loads these via `include_str!`, replaces the
embedded `cch=<value>` with the `cch=00000` sentinel, recomputes the checksum
with CCP's own `claude_code_cch_checksum`, and asserts it reproduces the
captured value. This proves CCP's body serialization + cch algorithm match real
Claude Code traffic **over a full-shape body** (`metadata`,
`context_management`, `thinking`, `tools`, `cache_control`, multiple `system`
blocks) — not just hand-minimized bodies and not CCP's own re-serialized output
...
| clean `HOME` path (3 encodings) | `/home/ccp-vector` | random + uid-bearing |
