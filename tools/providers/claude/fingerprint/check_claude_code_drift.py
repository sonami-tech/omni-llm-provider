...
PINNED_VERSION = "2.1.211"
PINNED_PROFILE = "cc-2.1.211-sdk-cli"
...
NO_CCH_VERSIONS = {"2.1.186", "2.1.197", "2.1.207", "2.1.211"}
...
# stable run-to-run and still exercises Omni's serializer against Claude Code's
...
PLACEHOLDER_HOME = "/home/omni-vector"
...
def normalize_vector_body(raw: bytes, home: str) -> bytes:
	...
	# HOME, deterministic):
	#   1. plain absolute path        (e.g. <tmpfs>/omni-vector-home)
...
def make_clean_home() -> str:
	...
	for base in (os.environ.get("XDG_RUNTIME_DIR"), "/dev/shm", f"/run/user/{os.getuid()}"):
		if base and os.path.isdir(base) and os.access(base, os.W_OK):
			home = os.path.join(base, "omni-vector-home")
			break
	else:
		raise RuntimeError(
...
def emit_vectors(claude_bin: str, version: str, out_dir: str, timeout: int) -> int:
	...
	vectors. Each vector is a real Claude Code body whose embedded cch Omni must
	reproduce - breaking the self-consistency circularity of hashing only Omni's
