...
# login. Uses Omni's own extract_flow.py for the report.
set -euo pipefail
...
if [ -n "${OMNI_CLAUDE_CAPTURE_PORT:-}" ]; then
  PORT="$OMNI_CLAUDE_CAPTURE_PORT"
else
...
WORKDIR="$(mktemp -d "$WORKBASE/omni-claude-baseline.XXXXXX")"
