...
def test_models_metadata():
	...
	assert "claude-opus-4-6" in names, f"Missing opus in {names}"
	...
	opus = models_by_id["claude-opus-4-6"]
...
@test("Model alias: 'opus' resolves to claude-opus-4-6")
def test_alias_opus():
	resp = client.chat.completions.create(
		model="opus",
		messages=[{"role": "user", "content": "Reply PONG"}],
	...
	assert "opus" in resp.model, f"Expected opus in model, got: {resp.model}"
	assert "PONG" in resp.choices[0].message.content
